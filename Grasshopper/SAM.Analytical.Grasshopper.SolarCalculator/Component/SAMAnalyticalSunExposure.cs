...
namespace SAM.Analytical.Grasshopper.SolarCalculator
{
    public class SAMGeometrySunExposure : GH_SAMVariableOutputParameterComponent
    {
        ...
        protected override void SolveInstance(IGH_DataAccess dataAccess)
        {
            ...
            if (index != -1)
            {
                dataAccess.SetData(index, Core.Query.Round(factor * 100, 0.01));
            }
            ...
            if(apertures != null)
            {
                foreach(Aperture aperture in apertures)
                {
                    ...
                    percents.Add(Core.Query.Round(factor * 100, 0.01));
                    if(face3Ds != null)
                    {
