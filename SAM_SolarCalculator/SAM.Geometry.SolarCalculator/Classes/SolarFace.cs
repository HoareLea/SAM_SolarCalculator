...
namespace SAM.Geometry.SolarCalculator
{
    public class SolarFace : Core.IJSAMObject, IFace3DObject 
    {
        ...
        public SolarFace(Guid guid, Face3D face3D)
        {
            this.guid = guid;
        ...
        public SolarFace(SolarFace solarFace)
        {
            if(solarFace == null)
            {
                return;
            ...
            guid = solarFace.guid;
            if(solarFace.face3D != null)
            {
                face3D = new Face3D(solarFace.face3D);
                boundingBox3D = face3D?.GetBoundingBox();
            }
        }

        public SolarFace(JObject jObject)
        {
            FromJObject(jObject);
        }
